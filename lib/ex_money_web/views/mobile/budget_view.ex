defmodule ExMoney.Web.Mobile.BudgetView do
  ...
  def build_linear_gradient(%{limit: limit, amount: amount} = category)
      when not is_nil(limit) and amount > limit do
    """
    background: linear-gradient(
      to right,
  ...
  def categories_chart_data(categories, limits) do
    ...
    categories =
      Enum.map(categories, fn {category, amount} ->
        category
        |> Map.from_struct()
    ...
    categories_tree =
      Enum.reduce(parents, [], fn parent_category, acc ->
        children =
          Enum.filter(subcategories, fn subcategory ->
            subcategory.parent_id == parent_category.id
          end)

        children =
          if parent_category[:amount] do
        ...
        children =
          Enum.map(children, fn child ->
            {float_amount, _} =
              Decimal.to_string(child[:amount], :normal)
              |> Float.parse()
  ...
  defp add_width(category, max_amount) do
    percent =
      (category[:amount] * 100 / max_amount)
      |> Float.round(0)
      |> :erlang.float_to_binary(decimals: 0)

    category =
      Map.put(category, :html_name, String.replace(category[:humanized_name], " ", "&nbsp;"))
  ...
  def expenses(transactions) do
    transactions
    |> Enum.reject(fn tr ->
      Decimal.compare(tr.amount, Decimal.new(0)) != Decimal.new(-1) or
        tr.category.name == "withdraw"
    end)
