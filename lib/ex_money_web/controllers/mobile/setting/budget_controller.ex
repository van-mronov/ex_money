defmodule ExMoney.Web.Mobile.Setting.BudgetController do
  ...
  def show(conn, _params) do
    ...
    if budget_template do
      ...
      items =
        Enum.sort(budget_template.items, fn item_1, item_2 ->
  ...
  def create(conn, params) do
    ...
    Repo.transaction(fn ->
      budget_template =
        BudgetTemplate.changeset(%BudgetTemplate{}, budget_template_params) |> Repo.insert!()

      Enum.each(params["budget_items"] || [], fn {category_id, amount} ->
        BudgetItem.changeset(%BudgetItem{}, %{
  ...
  def update(conn, params) do
    ...
    Repo.transaction(fn ->
      budget_template =
        BudgetTemplate.changeset(budget_template, params["budget_template"]) |> Repo.update!()
  ...
  def apply(conn, _params) do
    ...
    items =
      Enum.reduce(budget_template.items, %{}, fn item, acc ->
  ...
  defp categories_list do
    ...
    Enum.reduce(categories_dict, %{}, fn category, acc ->
      if is_nil(category.parent_id) do
        sub_categories =
          Enum.filter(categories_dict, fn c -> c.parent_id == category.id end)
