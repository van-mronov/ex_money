defmodule ExMoney.InteractiveChannel do
  ...
  def join("login_refresh:interactive", _message, socket) do
    ...
    with {true, interactive_fields_names} <- ongoing_interactive?(user_id) do
      Process.send_after(
        self(),
        {:interactive_callback_received, nil, interactive_fields_names},
        10
      )
    end
  ...
  def handle_info({:transactions_fetched, account_name, stored_transactions}, socket) do
    msg =
      case stored_transactions do
        0 -> "There are no new transactions"
        new -> "You've got #{new} new transaction(s)"
      end

    push(socket, "transactions_fetched", %{
      message: msg,
      title: "#{account_name} account got synced"
    })
  ...
  def handle_in("send_refresh_request", %{"login_id" => saltedge_login_id}, socket) do
    ...
    case result do
      {:error, _reason} ->
        push(socket, "refresh_request_failed", %{
          msg: "Could not refresh login.<br/> Try again later."
        })
      ...
      {:ok, _response} ->
        ...
        push(socket, "refresh_request_ok", %{
          msg: "Request has been sent.<br/> Waiting for a response..."
        })
    end
  ...
  def handle_in(
        "send_otp",
        %{"otp" => otp, "login_id" => saltedge_login_id, "field" => field},
        socket
      ) do
    body = """
    ...
    {:ok, response} =
      ExMoney.Saltedge.Client.request(:put, "logins/#{saltedge_login_id}/interactive", body)

    Logger.info("OTP has been send with the following result => #{inspect(response)}")

    push(socket, "otp_sent", %{
      title: "Transactions will by synced shortly",
