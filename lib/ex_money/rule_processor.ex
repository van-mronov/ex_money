defmodule ExMoney.RuleProcessor do
    ...
    transactions =
      Repo.all(
        from tr in Transaction,
          ...
          where:
            fragment("description ~* ?", ^rule.pattern) or
              fragment("extra->>'payee' ~* ?", ^rule.pattern)
      )
  ...
  defp transaction_description(transaction, extra) when extra == %{} or is_nil(extra),
    do: transaction.description
